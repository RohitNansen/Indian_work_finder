...
The code is in a public Git repository and is prepared for one Python process, SQLite and systemd timers. The exact commands depend on the VM distribution, domain and SSH setup. Keep the resume, database, provider keys and passwords outside Git. Do not paste replacement keys into chat.
...
2. Clone the Git repository into that directory.
...
6. Install `deploy/*.service` and `deploy/*.timer` into `/etc/systemd/system/`, adjusting paths and user if needed. Enable `job-finder.service`, `job-finder-alerts.timer`, `job-finder-quota.timer` and `job-finder-backup.timer`.
...
- Set `QUOTA_ALERT_EMAIL` to the owner's address in the private `.env`. The quota timer checks JSearch, the OpenRouter API key, the app's own spending cap, and email counts hourly. It attempts emails at 80%, 95% and 100%, once per billing period and threshold, and records success or failure on `/activity`. Resend requires a verified sender (`EMAIL_FROM`) and `RESEND_API_KEY`; at a Resend hard limit an email may fail, so check the in-app log too. If an OpenRouter API key has no spending cap, its endpoint does not expose remaining account credits. Set a limit on that key or watch provider error alerts for credit exhaustion.
